...
const ChatRoom = () => {
  ...
  return <div className="bg-black w-[45vw]">
    ...
    <div className="m-2">
      {tab == 0 ? <div>
        <div className="flex gap-4">
          <span className="text-green-400">Online: {players ? players.length : 0}</span>
          <span className="text-yellow-500">Playing: {players ? players.length : 0}</span>
        </div>
        ...
        <div className="m-2">
          {players?.map((item, index) => <div key={index} className={`grid grid-cols-4 gap-4 ${item.cashout ? "text-green-500" : "text-red-500"}`}>
            <div>{item.user} </div>
            <div>{item.stake}</div>
            <div>{item.odd}x</div>
            <div>{item.won}</div>
      ...
      {tab == 1 ? <div> Histroy </div> : null}
      {tab == 2 ? <div> Chats </div> : null}
    </div>
  </div >
